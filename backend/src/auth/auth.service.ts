...
export class AuthService {
    ...
    // ─── Owner Sign-Up ────────────────────────────────────────────────────
    async signUpOwner(name: string, email: string, password: string, companyName: string) {
    ...
    async completeSignUp(email: string, name: string, password: string) {
        ...
        if (!user) {
            throw new NotFoundException('Invitation not found. Please ask your admin to re-invite you.');
        }

        if (user.passwordHash) {
            throw new BadRequestException('This account has already been set up. Please sign in.');
        }
    ...
    // ─── Helpers ──────────────────────────────────────────────────────────
    private createToken(user: { id: string; email: string; role: string; companyId: string }): string {
