...
function VerifyForm() {
  ...
  async function handleResend() {
    ...
    try {
      ...
      setCountdown(600);
      setSuccess('Um novo código foi enviado.');
        ...
        <div className="text-center">
          {countdown > 0 ? (
            <p className="text-sm text-blue-300/40">
              Código expira em{' '}
              <span
                className={`font-mono font-medium ${countdown < 60 ? 'text-red-400' : 'text-blue-300'}`}
              >
                {formatTime(countdown)}
              </span>
            </p>
          ) : (
            <p className="text-sm text-red-400 font-medium">Código expirado</p>
          )}
        </div>
