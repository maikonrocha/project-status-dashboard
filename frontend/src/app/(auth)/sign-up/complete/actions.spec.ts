...
describe('completeSignUpAction', () => {
    it('returns error on API failure', async () => {
        mockApiFetch.mockResolvedValue({ ok: false, status: 400, data: { message: 'Token inválido.' } });

        const result = await completeSignUpAction({ email: 'a@b.com', name: 'A', password: 'p' });
    ...
    it('returns { success: true } on success', async () => {
        ...
        const result = await completeSignUpAction({ email: 'a@b.com', name: 'Alice', password: 'pass' });
