...
describe('signUpAction', () => {
    it('returns error when API fails', async () => {
        mockApiFetch.mockResolvedValue({ ok: false, status: 409, data: { message: 'Email já cadastrado.' } });

        const result = await signUpAction({ name: 'A', email: 'a@b.com', password: 'p', companyName: 'Co' });
    ...
    it('returns { success: true } on success', async () => {
        ...
        const result = await signUpAction({ name: 'Alice', email: 'a@b.com', password: 'pass', companyName: 'Acme' });
        expect(result).toEqual({ success: true });
    });

    it('posts to /auth/sign-up with the provided data', async () => {
        mockApiFetch.mockResolvedValue({ ok: true, status: 201, data: {} });
        const payload = { name: 'Bob', email: 'b@c.com', password: '123', companyName: 'Corp' };
