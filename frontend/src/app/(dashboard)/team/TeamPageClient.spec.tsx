...
describe('TeamPageClient — empty state', () => {
  it('shows empty state message when no members', () => {
    ...
    expect(screen.getByText(/Nenhum membro ainda/i)).toBeInTheDocument();
  });
});
...
describe('TeamPageClient — members table', () => {
  ...
  it('shows Pending status for unverified members', () => {
    render(<TeamPageClient initialMembers={members} companyName="Acme" />);
    const pendingElements = screen.getAllByText('Pendente');
...
describe('TeamPageClient — invite form', () => {
  it('renders invite email input and Send Invite button', () => {
    render(<TeamPageClient initialMembers={[]} companyName="Acme" />);
    expect(
      screen.getByPlaceholderText(/colega@empresa.com/i),
    ).toBeInTheDocument();
    expect(
  ...
  it('shows success message after successful invite', async () => {
    ...
    await userEvent.type(
      screen.getByPlaceholderText(/colega/i),
      'new@user.com',
    );
    ...
    await waitFor(() =>
      expect(
        screen.getByText(/Convite enviado para new@user.com/),
      ).toBeInTheDocument(),
    );
  });
  ...
  it('clears the email input after successful invite', async () => {
    ...
    const input = screen.getByPlaceholderText(/colega/i) as HTMLInputElement;
  ...
  it('shows error message when invite fails', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText('Email já convidado.')).toBeInTheDocument(),
    );
  });

  it('disables button while invite is in progress', async () => {
    let resolve!: (v: Awaited<ReturnType<typeof inviteAction>>) => void;
    mockInviteAction.mockReturnValue(
      new Promise((r) => {
        resolve = r;
      }),
    );
    ...
    await userEvent.type(screen.getByPlaceholderText(/colega/i), 'x@y.com');
    await userEvent.click(
      screen.getByRole('button', { name: /Enviar Convite/i }),
    );
