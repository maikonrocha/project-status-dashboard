...
import { authApi, type TeamMember } from '@/lib/api-client';
import { useAuth } from '@/lib/auth-context';

export default function TeamPage() {
    const { isOwner, isAuthenticated, user } = useAuth();
    ...
    useEffect(() => {
        if (!isAuthenticated || !isOwner) {
            if (!isOwner && isAuthenticated) router.push('/');
            return;
        }
    ...
    async function handleInvite(e: React.FormEvent) {
        ...
        try {
            await authApi.invite({ email: inviteEmail });
        ...
        <main className="max-w-4xl mx-auto px-6 py-10">
            ...
            <div className="mb-8">
                ...
                <p className="text-blue-300/50 text-sm mt-1">
                    Manage your company&apos;s team members ({user?.companyName})
                </p>
            </div>
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="border-b border-white/10 bg-white/5">
                            <th className="text-left px-5 py-3 text-blue-300/60 font-medium">Member</th>
                            <th className="text-left px-5 py-3 text-blue-300/60 font-medium hidden md:table-cell">Role</th>
                            <th className="text-left px-5 py-3 text-blue-300/60 font-medium hidden md:table-cell">Status</th>
                            <th className="text-left px-5 py-3 text-blue-300/60 font-medium hidden lg:table-cell">Joined</th>
                        </tr>
                    </thead>
                    <tbody>
                        {members.map((member) => (
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p className="font-medium text-white">
                                                {member.name || <span className="text-white/40 italic">Pending invite</span>}
                                            </p>
